...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (androidElement && iosElement) {
        ...
        Promise.all(urls.map(url =>
            fetch(url)
                .then(response => Object.create({ url: url, result: response.json() }))
        )).then(async dataArray => {
            var android = dataArray[0];
            var ios = dataArray[1];
            for (let i = 0; i < dataArray.length; i++) {
                let item = dataArray[i]
                if (item.url == urls[0]) {
                    android = await item.result;
                }
                if (item.url == urls[1]) {
                    ios = await item.result;
                }
            }
...
function setCommitInfo(data, element) {
    ...
    listItem.textContent = `commit message: ${lastCommit.commit.message}`;
    const listItem2 = document.createElement('li');
    listItem2.textContent = `Ngày commit: ${formatTime(commitDate)}`;
